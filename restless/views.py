...
#from django.utils import simplejson as json depracated
...
def validate_request(requestdata):
    ...
    if ApplicationLink.objects.filter(client_name = requestdata["client_id"], server_name = settings.SITE_NAME).exists():
        applink = ApplicationLink.objects.get(client_name = requestdata["client_id"], server_name = settings.SITE_NAME)
    # Validate the secret
