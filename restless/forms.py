...
def retrieve_access_token(request, service):
    ...
    '''
    applink = ApplicationLink.objects.filter(client_name=settings.SITE_NAME,
        server_name__istartswith=service).order_by("-modified")[0]
...
class GoldenEyeWidget(forms.Textarea):
    ...
    def render(self, name, value, attrs=None):
        serviceurl, access_token = retrieve_access_token(self.request, "ge")
...
def GoldenEyeViewer(request, features=[], geom_field="shape", instance="cc/instances/public", html_info=None):
    serviceurl, access_token = retrieve_access_token(request, "ge")
