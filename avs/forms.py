...
class PilotFilterDRForm(DateRangeForm):
    pilot = forms.ModelMultipleChoiceField(
        queryset=Pilot.objects.all().order_by("first_name"),
        required=False,
        label='Pilots',
...
class FlightLogFieldSearch(FieldFilterDRForm):
    flight_log_number = forms.CharField(
        required=False,
        label="Flight Log Number",
        widget=forms.TextInput(
            attrs={
                'style': 'width:150px',
            }))
    fire_number = forms.CharField(
        required=False,
        label="Fire Number",
        widget=forms.TextInput(
            attrs={
...
class AircraftFlightLogForm(BaseForm):
    fire_danger_index = forms.IntegerField(
        label='Fire Danger Index', required=False)
    aircraft = forms.ModelChoiceField(
        queryset=Aircraft.objects.filter(
            effective_to__exact=None), empty_label="", widget=forms.Select(
            attrs={
                'style': 'width:150px', }))
...
class AircraftFlightLogDetailForm(BaseForm):
    task = forms.ModelChoiceField(
        ...
        empty_label="")
    pilot_in_command = forms.ModelChoiceField(
        queryset=Pilot.objects.filter(
            effective_to__exact=None), empty_label="")
    pilot_in_command_under_supervision = forms.ModelChoiceField(
        queryset=Pilot.objects.filter(
            effective_to__exact=None), empty_label="", required=False)
...
class DutyTimeForm(BaseForm):
    ...
    datetime_on_first = forms.TimeField(
        ('%H%M',), label='Time On (24)', required=False, widget=forms.TimeInput(
            format='%H%M', attrs={
                'class': 'input', }))
    datetime_off_first = forms.TimeField(
        ('%H%M',), label='Time Off (24)', required=False, widget=forms.TimeInput(
            format='%H%M', attrs={
                'class': 'input', }))
